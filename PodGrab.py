...
#        item_file_name = item_file_name[:50]
	...
	local_file = current_directory + os.sep + DOWNLOAD_DIRECTORY + os.sep + channel_title + os.sep + \
			timeNow.strftime('%Y%m%d%H%M') + clean_string(item_file_name)
...
def iterate_channel(chan, today, mode, cur, conn, feed, channel_title):
	...
	for item in chan.getElementsByTagName('item'):
		try:
			...
			today2 = strftime('%a, %d %b %Y', gmtime())
			struct_time_today = strptime(today2, "%a, %d %b %Y")
			try:
			...
			if not has_error:
				if mktime(struct_time_item) > mktime(struct_last_ep) or mode == MODE_DOWNLOAD or (channel_title == 'Forum-La1ere' and mktime(struct_time_item)>mktime(struct_time_today)):
